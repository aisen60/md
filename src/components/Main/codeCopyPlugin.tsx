  ...
  return {
    viewerEffect({ markdownBody }) {
      ...
      pres.forEach((pre: HTMLPreElement) => {
        pre.className = "clipboard-container";
        pre.setAttribute("data-check", "false");
        ...
        clipboardNode.addEventListener("click", () => {
          const checkStatus = pre.getAttribute("data-check");
          if (checkStatus !== "false") return;

          pre.setAttribute("data-check", "true");

          const svgNodes = pre.querySelectorAll("svg");
          ...
          const codeNode = pre.querySelector("code") as HTMLElement;
          copyToClipboard(codeNode.innerText);
          toast.success("复制成功");
        });

        pre.appendChild(clipboardNode);
      });
    },
  };
}
